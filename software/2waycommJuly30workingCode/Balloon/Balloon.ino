...
#define pL 5    //Maxumimum packet length
                                        /******GLOBAL VARIABLES***********/
/**************************************************************************************************************/
...
void loop (void)
{
  while(1)//infinite program loop
  {
    ...
    if(!emT)//if the transmission buffer is not empty this means only transmits when there is something to transmit
    {
     ...
     /***Note "source" is a flag based on previously recved data  and its value is updated every
     half second when something is popped from the receive stack
     */
     /*data from recieve stack  a usually processed durring the interrupt in order for the main
     but the process below requires the the use of the I2C sub system therefore it cannot be processed durring the interrupt*/
     if(source==1) //checking  if there is a message from source flag
     {  
       source=0;// sets the source back to zero
       Serial.print("light:");      
       //preparing  to tranmit back in response to a recieved message 
       TXS[tsp][0]=1;//Identifying itself at the beginning of  the  packet the first byte is for source
       TXS[tsp][1]=10;//futher identification 
       TXS[tsp][2]=collect2(5,1);//getting 1 byte frome preipherral 5 to be transmitted through I2C interface
...
ISR(TIMER1_COMPA_vect)//first timer interrup
{
  ...
  if(source==2)//checking the source for the request
  {
    ...
    float input=(float)analogRead(1);//reading analog data 
    input=input/4.0; //converting it to a byte long
    TXS[tsp][0]=2;//setting identifying preambles 
    TXS[tsp][1]=20;
    TXS[tsp][2]=(byte)input; // placing the message where it will go    
    updateT();
    if(!fulT)
    {
    ...
    tsp=inc(tsp);//updating the space index
    fulT=enqueueT();//updating Buffer indexes 
  }
 /****periodic transmissions ***/ 
  if(isc%20==0)//the if statments checks for every 10 seconds sinc in this set up isc updates twice in a second 
  {
    /*This is an example of a tranmission message that can be done in a Interrupt because it  does not contain the use of the I2C sub system*/    
    TXS[tsp][0]=3;
    TXS[tsp][1]=1;
    TXS[tsp][2]=collect(3); //collecting data from SPI periferral enabled by pin 3    
    updateT();
    if(!fulT)
    {
      TX_Buffer[tsi].content=TXS[tsp];
 ...
 /***below is an example of how to activate flags for periodic messages with function calls no allowed in an interrupt**/ 
  if(isc%30==0)
  {
    /* this sets the flag for an automatic message in the main loop to activate every 15
    seconds which must be set back to zero after the if statement of the flag is read
  */
    auto1=1;//setting the flag
  }
